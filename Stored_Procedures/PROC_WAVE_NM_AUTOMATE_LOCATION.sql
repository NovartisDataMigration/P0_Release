...
IF OBJECT_ID('dbo.GetLocationAddress') IS NOT NULL
  DROP FUNCTION GetLocationAddress
GO
CREATE FUNCTION GetLocationAddress (
    @ENTITY              AS NVARCHAR(2000),
    @LOC_STREET1         AS NVARCHAR(2000),
	@LOC_STREET2         AS NVARCHAR(2000),
	@LOC_STREET3         AS NVARCHAR(2000),
	@LOC_TOWN            AS NVARCHAR(2000),
	@MAIL_BOX            AS NVARCHAR(2000),
	@LOC_REGION          AS NVARCHAR(2000)
)
RETURNS NVARCHAR(500)  
BEGIN  
     SELECT * FROM [SET_ADDRESS_FIELD_MAPPING_LOCATION] WHERE  
END
GO
...
--SELECT DISTINCT * FROM NOVARTIS_DATA_MIGRATION_ERROR_LIST WHERE Wave='P0' AND Report='Location'
--SELECT DISTINCT [Country Code] FROM NOVARTIS_DATA_MIGRATION_ERROR_LIST WHERE Wave='P0' AND Report='Location' AND [Error Text] like '%Postal Code is missing;%'
...
--                    ,'CHE' as ISO3 -- ,RTRIM(LTRIM(ISNULL((SELECT [ISO3] FROM COUNTRY_LKUP WHERE [ISO2]=A3.[ISO code]), ''))) ISO3  
--                    ,'Forum 1' as LOC_STREET1--,RTRIM(LTRIM(ISNULL(A3.[Location Street 1], ''))) LOC_STREET1
--                    ,'Novartis Campus' as LOC_STREET2--,RTRIM(LTRIM(ISNULL(A3.[Location Street 2], ''))) LOC_STREET2
--                    ,'' LOC_STREET3--,RTRIM(LTRIM(ISNULL(A3.[Location Street 3], ''))) LOC_STREET3
--                    ,'Basel' LOC_TOWN--,RTRIM(LTRIM(ISNULL(A3.[Location Town], ''))) LOC_TOWN
--                    ,'CHE-BS' LOC_REGION--,IIF(A2.[Region(State)]='Required', 'NVS', '') LOC_REGION
--                    ,'' LOC_CITYSUBDIVISION--,IIF(A2.[City Subdivision]='Required', 'Default', '') LOC_CITYSUBDIVISION
--                    ,'' LOC_REGIONSUBDIVISION--,IIF(A2.[Subregion(Country)]='Required', 'Default', '') LOC_REGIONSUBDIVISION
--                    ,'4056' MAIL_BOX--,IIF(RTRIM(LTRIM(ISNULL(A3.[Mail P.O. Box], '')))='', IIF(A2.[Postal Code]='Not Accepted', '', ISNULL(A2.[Postal Code Default Values], '')), RTRIM(LTRIM(ISNULL(A3.[Mail P.O. Box], '')))) MAIL_BOX			   
...
--) A10 WHERE RNUM = 1
	...
	BEGIN TRY 
		...
		--select * from P0_YPAXX_SITE_CODE
		...
		DECLARE @LOCATION_SOURCE TABLE (
              ...
              [LOC_CITYSUBDIVISION]  NVARCHAR(2000),
              [LOC_REGIONSUBDIVISION]  NVARCHAR(2000),
		       ...
		       * FROM (
                   SELECT *, ROW_NUMBER() OVER(PARTITION BY [LOCATION ID] ORDER BY [LOCATION ID]) RNUM FROM (
                        SELECT DISTINCT 
                            ...
                            ,RTRIM(LTRIM(ISNULL(A3.[Location Street 1], ''))) LOC_STREET1
                            ,RTRIM(LTRIM(ISNULL(A3.[Location Street 2], ''))) LOC_STREET2
                            ,RTRIM(LTRIM(ISNULL(A3.[Location Street 3], ''))) LOC_STREET3
                            ,RTRIM(LTRIM(ISNULL(A3.[Location Town], ''))) LOC_TOWN
                            ,IIF(A2.[Region(State)]='Required', 'NVS', '') LOC_REGION
                            ,IIF(A2.[City Subdivision]='Required', 'Default', '') LOC_CITYSUBDIVISION
                            ,IIF(A2.[Subregion(Country)]='Required', 'Default', '') LOC_REGIONSUBDIVISION
                            ,IIF(RTRIM(LTRIM(ISNULL(A3.[Mail P.O. Box], '')))='', IIF(A2.[Postal Code]='Not Accepted', '', ISNULL(A2.[Postal Code Default Values], '')), RTRIM(LTRIM(ISNULL(A3.[Mail P.O. Box], '')))) MAIL_BOX			   
                            ,RTRIM(LTRIM(ISNULL(A3.[BUSINESS UNIT CODE], ''))) [BUSINESS UNIT CODE]		   
                        FROM P0_YPAXX_SITE_CODE A3 LEFT JOIN WAVE_ADDRESS_VALIDATION A2 ON A2.[Country2 Code]=A3.[ISO code]
                        WHERE (A3.[End Date] >= CAST(@which_date as date) and A3.[Start Date] <= CAST(@which_date as date))
                   ) R1 			
		...
		--UPDATE @LOCATION_SOURCE SET [ISO3]='CHE', [LOC_STREET1]='Forum 1', [LOC_STREET2]='Novartis Campus', [LOC_STREET3]='', [LOC_TOWN]='Basel', [LOC_REGION]='CHE-BS', [MAIL_BOX]='4056'
		...
		*/
	    ...
	    SELECT * FROM (
			SELECT @which_wavestage Wave
				  ...
				  ,(
                     ...
                     IIF(ISNULL([MAIL_BOX], '')='' AND (ISNULL(A2.[Postal Code], '')='Required' OR ISNULL(A2.[Postal Code], '')='Optional'), ' Postal Code is missing;', '')+
		...
		SELECT
			...
			,'CHE' [Country Reference ID]
			,''[Last Modified]
			,'Basel'/*ISNULL(LOC_TOWN, '')*/ [Municipality]
			...
			,'CHE-BS'/*ISNULL([LOC_REGION], '')*/ [Country Region Reference ID]
			,''[Country Region Descriptor]
			,'4056'/*ISNULL(MAIL_BOX, '')*/ [Postal Code]
			,'TRUE'[Public]
			,'TRUE'[Primary]
		...
		SELECT
		     [LOCATION KEY] [Location Key]
			,'' [Address Data Key]
			,ROW_NUMBER() OVER(PARTITION BY [Location Key], [Address Data Key] ORDER BY [Location Key], [Address Data Key],'' [LOC_STREET] /* adding this for p0*/) [Address Line Data Key]
		...
		--UPDATE WD_HR_TR_AUTOMATED_LOCATION_ADDRESS_LINE_DATA SET [Address Line Data] = 'Forum 1' WHERE [Address Line Data Type] = 'ADDRESS_LINE_1';
		--UPDATE WD_HR_TR_AUTOMATED_LOCATION_ADDRESS_LINE_DATA SET [Address Line Data] = 'Novartis Campus' WHERE [Address Line Data Type] = 'ADDRESS_LINE_2';		
		...
		SELECT
			...
			,IIF(LOC_CITYSUBDIVISION='', '', 'CITY_SUBDIVISION_1') [Submunicipality Data Type]
			,LOC_CITYSUBDIVISION [Submunicipality Data]
		FROM (SELECT DISTINCT [LOCATION KEY], [ADDRESS DATA KEY], [LOC_STREET1], [LOC_STREET2], [LOC_STREET3], [LOC_TOWN], [LOC_CITYSUBDIVISION], [MAIL_BOX]  FROM P0_LOCATION_SOURCE) A6
		...
		SELECT
			...
			,IIF(LOC_REGIONSUBDIVISION='', '','REGION_SUBDIVISION_1') [Subregion Data Type]
			,'' [LOC_REGIONSUBDIVISION] [Subregion Data]
		FROM (SELECT DISTINCT [LOCATION KEY], [ADDRESS DATA KEY], [LOC_STREET1], [LOC_STREET2], [LOC_STREET3], [LOC_TOWN], [LOC_REGIONSUBDIVISION], [MAIL_BOX]  FROM P0_LOCATION_SOURCE) A6
