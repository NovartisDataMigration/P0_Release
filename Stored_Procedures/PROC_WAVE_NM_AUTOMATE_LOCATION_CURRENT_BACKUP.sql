...
BEGIN  
     ...
     DECLARE cursor_item CURSOR FOR SELECT SRC_MAP_FIELD 
	                                    FROM [SET_ADDRESS_FIELD_MAPPING_LOCATION] 
										WHERE ISNULL(SRC_MAP_FIELD, '') <> '' AND ISO2=@ISO2 ORDER BY [INDEX]
	 OPEN cursor_item;
	 FETCH NEXT FROM cursor_item INTO @SrcMapField;
	 WHILE @@FETCH_STATUS = 0
	 BEGIN
	    SET @ReturnValue = @ReturnValue + (CASE 
		                       WHEN @SrcMapField='_addressLine1' THEN IIF(ISNULL(@ADDRESS_LINE1, '')='', '', ','+@ADDRESS_LINE1)
							   WHEN @SrcMapField='_addressLine2' THEN IIF(ISNULL(@ADDRESS_LINE2, '')='', '', ','+@ADDRESS_LINE2)
							   WHEN @SrcMapField='_addressLine3' THEN IIF(ISNULL(@ADDRESS_LINE3, '')='', '', ','+@ADDRESS_LINE3)
							   WHEN @SrcMapField='_addressLine4' THEN IIF(ISNULL(@ADDRESS_LINE4, '')='', '', ','+@ADDRESS_LINE4)
							   WHEN @SrcMapField='_addressLine5' THEN IIF(ISNULL(@ADDRESS_LINE5, '')='', '', ','+@ADDRESS_LINE5)
							   WHEN @SrcMapField='_addressLine6' THEN IIF(ISNULL(@ADDRESS_LINE6, '')='', '', ','+@ADDRESS_LINE6)
							   WHEN @SrcMapField='_addressLine7' THEN IIF(ISNULL(@ADDRESS_LINE7, '')='', '', ','+@ADDRESS_LINE7)
							   WHEN @SrcMapField='_addressLine8' THEN IIF(ISNULL(@ADDRESS_LINE8, '')='', '', ','+@ADDRESS_LINE8)
							   WHEN @SrcMapField='_addressLine9' THEN IIF(ISNULL(@ADDRESS_LINE9, '')='', '', ','+@ADDRESS_LINE9)
							   WHEN @SrcMapField='_city' THEN IIF(ISNULL(@CITY, '')='', '', ','+@CITY)
							   WHEN @SrcMapField='_region' THEN IIF(ISNULL(@REGION, '')='', '', ','+@REGION)
							   WHEN @SrcMapField='_postalcode' THEN IIF(ISNULL(@POSTAL_CODE, '')='', '', ','+@POSTAL_CODE)
							   WHEN @SrcMapField='_City_Subdivision_1' THEN IIF(ISNULL(@CITY_SUBDIVISION_1, '')='', '', ','+@CITY_SUBDIVISION_1)
							   WHEN @SrcMapField='_City_Subdivision_2' THEN IIF(ISNULL(@CITY_SUBDIVISION_2, '')='', '', ','+@CITY_SUBDIVISION_2)
							   WHEN @SrcMapField='_Region_Subdivision_1' THEN IIF(ISNULL(@REGION_SUBDIVISION_1, '')='', '', ','+@REGION_SUBDIVISION_1)
							   WHEN @SrcMapField='_Region_Subdivision_2' THEN IIF(ISNULL(@REGION_SUBDIVISION_2, '')='', '', ','+@REGION_SUBDIVISION_2)
							END)
     	FETCH NEXT FROM cursor_item INTO @SrcMapField;
	END
	CLOSE cursor_item; 
                   ...
                   SELECT *, ROW_NUMBER() OVER(PARTITION BY [LOCATION ID] ORDER BY [LOCATION ID]) RNUM FROM (
                        SELECT DISTINCT 
                            ...
                            ,RTRIM(LTRIM(ISNULL(dbo.GetAddressInfo(A3.[ISO code], '_addressLine1', 
																   ...
																   A3.[Location Town],
																   IIF(A2.[Region(State)]='Required', 'NVS', ''),
																   IIF(A2.[City Subdivision]='Required', 'Default', ''),
																   IIF(A2.[City Subdivision 1]='Required', 'Default', ''),
																   IIF(A2.[Region Subdivision 1]='Required', 'Default', ''),
																   IIF(A2.[Region Subdivision 2]='Required', 'Default', ''),
																   IIF(RTRIM(LTRIM(ISNULL(A3.[Mail P.O. Box], '')))='', 
																         IIF(A2.[Postal Code]='Not Accepted', '', ISNULL(A2.[Postal Code Default Values], '')), 
